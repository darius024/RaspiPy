...
uint32_t instructionToInt(uint32_t addr) {
    uint32_t result = 0;
    result |= mem[addr];
    result |= (mem[addr + 1] << 8);
...
void writeOutput(const char *filename) {
    ...
    for (int addr = 0; addr < MEMORY_SIZE; addr += 4) {
        uint32_t binInstr = instructionToInt(addr);
        if (binInstr != 0) {
...
Instruction decodeDPImm(uint32_t instr) {
    struct DPI dpimm;
    ...
    Instruction instruction = {.instructionType = isDPI, .dpimm = dpimm};
    return instruction;
}
...
Instruction decodeDPR(uint32_t instr) {
    struct DPR dpr;
    ...
    } else if (dpr.opr != 8 && dpr.opc != 0) {
        perror("Invalid multiply instruction.");
        exit(EXIT_FAILURE);
    }

    Instruction instruction = {.instructionType = isDPR, .dpr = dpr};
    return instruction;
}
...
Instruction decodeSDT(uint32_t instr) {
    struct SDT sdt;
    ...
    if (sdt.mode == 1) {
        ...
        uint16_t offset = getBits(instr, 10, 12);
    ...
    Instruction instruction = {.instructionType = isSDT, .sdt = sdt};
    return instruction;
}
...
Instruction decodeB(uint32_t instr) {
    struct Branch br;
    ...
    Instruction instruction = {.instructionType = isBranch, .br = br};
    return instruction;
}
